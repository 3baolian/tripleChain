...
{
    QString ret = QString("triplechain:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non TBC unit input in lnReqAmount, we generate the URI with TBC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::TBC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
