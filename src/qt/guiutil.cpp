...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("triplechain"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::TBC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
{
    // Convert triplechain:// to triplechain:
    //
    //    Cannot handle this later, because triplechain:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("triplechain://"))
    {
        uri.replace(0, 12, "triplechain:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "triplechain.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for triplechain.lnk
...
{
    return GetAutostartDir() / "triplechain.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write an triplechain.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=triplechain\n";
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("TripleChain") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  triplechain [" + tr("command-line options") + "]                     " + "\n";
...
void SetBlackThemeQSS(QApplication& app)
{
    app.setStyleSheet("QWidget        { background: rgb(41,44,48); }"
                      ...
                      "QLineEdit:hover{ background: rgb(63,67,72); color: rgb(255,255,255); border-color: rgb(194,194,194); border: 2px solid rgb(194,194,194); }"
                      "QLineEdit:disabled{ background: rgb(63,67,72); color: rgb(255,255,255); border-color: rgb(194,194,194); border: 2px solid rgb(194,194,194); }"
                      "QTextEdit      { background: rgb(63,67,72); color: rgb(255,255,255); }"
                      "QTextEdit:hover{ background: rgb(63,67,72); color: rgb(255,255,255); border-color: rgb(194,194,194); border: 2px solid rgb(194,194,194); }"
                      "QTextEdit:disabled{ background: rgb(63,67,72); color: rgb(255,255,255); border-color: rgb(194,194,194); border: 2px solid rgb(194,194,194); }"
                      ...
                      "QGroupBox      { color: rgb(120,127,139); }"
