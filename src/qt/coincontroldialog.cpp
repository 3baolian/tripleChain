...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    foreach(const qint64 &amount, CoinControlDialog::payAmounts)
    {
        ...
        if (amount > 0)
        {
            ...
            CTxOut txout(amount, (CScript)vector<unsigned char>(24, 0), txDummy.nTime);
    ...
    int nDisplayUnit = BitcoinUnits::TBC;
    ...
    BOOST_FOREACH(PAIRTYPE(QString, vector<COutput>) coins, mapCoins)
    {
        ...
        BOOST_FOREACH(const COutput& out, coins.second)
        {
            ...
            if(ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show TripleChain address. In tree mode, address is not shown again for direct wallet address outputs
