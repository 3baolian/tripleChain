...
{
    resize(850+95, 550);
    setWindowTitle(tr("TripleChain") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/triplechain"));
    setWindowIcon(QIcon(":icons/triplechain"));
#else
    ...
    lockTransactionsPage = new QWidget(this);
    QVBoxLayout *vLockBox = new QVBoxLayout();
    lockTransactionView = new LockTransactionView(this);
    vLockBox->addWidget(lockTransactionView);
    lockTransactionsPage->setLayout(vLockBox);
    ...
    centralStackedWidget->addWidget(lockTransactionsPage);
    ...
    // Double-clicking on a transaction on the locked transaction history page shows details
    connect(lockTransactionView, SIGNAL(doubleClicked(QModelIndex)), lockTransactionView, SLOT(showDetails()));
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to an TripleChain address"));
    ...
    //lockHistoryAction = new QAction(QIcon(":/icons/lock_history"), tr("&Locked Transactions"), this);
    //lockHistoryAction->setToolTip(tr("Browse all locked transaction history"));
    //lockHistoryAction->setCheckable(true);
    //lockHistoryAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_5));
    //tabGroup->addAction(lockHistoryAction);
    ...
    //connect(lockHistoryAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
    //connect(lockHistoryAction, SIGNAL(triggered()), this, SLOT(gotoLockHistoryPage()));
    ...
    aboutAction = new QAction(tr("&About TripleChain"), this);
    aboutAction->setToolTip(tr("Show information about TripleChain"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for TripleChain"));
    optionsAction->setMenuRole(QAction::PreferencesRole);
    toggleHideAction = new QAction(QIcon(":/icons/triplechain"), tr("&Show / Hide"), this);
...
void BitcoinGUI::createToolBars()
{
    ...
    if (fUseBlackTheme)
    {
        ...
        //header->setMinimumSize(160, 116);
        //header->setSizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed);
        header->setStyleSheet("QWidget { background-color: rgb(24,26,30); background-repeat: no-repeat; background-image: url(:/images/header_w); background-position: top center; }");
        //toolbar->addWidget(header);
        //toolbar->addWidget(makeToolBarSpacer());
    }
	else
	{
		header->setStyleSheet("QWidget { background-color: #e8e8e8; background-repeat: no-repeat; background-image: url(:/images/header); background-position: top center; }");
	}
	toolbar->addWidget(header);
    ...
    //toolbar->addAction(lockHistoryAction);
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/triplechain_testnet"));
            setWindowIcon(QIcon(":icons/triplechain_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/triplechain_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("TripleChain client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::setWalletModel(WalletModel *walletModel)
{
    ...
    if(walletModel)
    {
        ...
        lockTransactionView->setModel(walletModel);
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("TripleChain client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to TripleChain network", "", count));
}
...
{
    QString strTitle = tr("TripleChain") + " - ";
...
void BitcoinGUI::gotoLockHistoryPage()
{
    //lockHistoryAction->setChecked(true);
    centralStackedWidget->setCurrentWidget(lockTransactionsPage);

    exportAction->setEnabled(true);
    disconnect(exportAction, SIGNAL(triggered()), 0, 0);
    connect(exportAction, SIGNAL(triggered()), lockTransactionView, SLOT(exportClicked()));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid TripleChain address or malformed URI parameters."));
    }
