    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    LogPrintf("WalletUpdateSpent found spent coin %s BLK %s\n", FormatMoney(wtx.GetCredit(true, true)), wtx.GetHash().ToString());
...
int64_t CWallet::GetLockedBalance() const
{
    int64_t nTotal = 0;
    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            const CWalletTx* pcoin = &(*it).second;
            if (pcoin->IsTrusted())
                nTotal += (CWallet::GetCredit(*pcoin, true) - CWallet::GetCredit(*pcoin));
        }
    }
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
                if (!(pcoin->IsSpent(i)) && IsMine(pcoin->vout[i]) && pcoin->vout[i].nValue >= nMinimumInputValue && !IsLockedTxOut(pcoin->vout[i]) &&
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                // vouts to the payees
                BOOST_FOREACH (const PAIRTYPE(CScript, int64_t)& s, vecSend)
                    wtxNew.vout.push_back(CTxOut(s.second, s.first, 0));
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-triplechain-address
                    ...
                    wtxNew.vout.insert(position, CTxOut(nChange, scriptChange, 0));
                }
                else
...
bool CWallet::CreateTransactionWithLock(const vector<pair<CScript, CLockData> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, const CCoinControl* coinControl)
{
    int64_t nValue = 0;
    BOOST_FOREACH(const PAIRTYPE(CScript, CLockData)& s, vecSend)
    {
        const CLockData& data = s.second;
        if(!MoneySendRange(data.nAmount) || !LockMonthRange(data.nMonth))
            return false;
        nValue += data.nAmount + GetUnlockReward(data.nAmount, data.nMonth);
    }

    if(vecSend.empty() || !MoneySendRange(nValue))
        return false;
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                {
                    const CLockData& data = s.second;
                    wtxNew.vout.push_back(CTxOut(data.nAmount + GetUnlockReward(data.nAmount, data.nMonth), s.first, nBestHeight + data.nMonth * 30 * 24 * 60));
                }
...
bool CWallet::CreateTransactionWithLock(CScript scriptPubKey, int64_t nLockAmount, int nLockMonth, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, const CCoinControl* coinControl)
{
    vector<pair<CScript, CLockData> > vecSend;
    vecSend.push_back(make_pair(scriptPubKey, CLockData(nLockAmount, nLockMonth)));
    return CreateTransactionWithLock(vecSend, wtxNew, reservekey, nFeeRet, coinControl);
}
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    CScript scriptEmpty;
    scriptEmpty.clear();
    txNew.vout.push_back(CTxOut(0, scriptEmpty, 0));
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        ...
        for (unsigned int n=0; n<min(nSearchInterval,(int64_t)nMaxStakeSearchInterval) && !fKernelFound && pindexPrev == pindexBest; n++)
        {
            ...
            if (CheckKernel(pindexPrev, nBits, txNew.nTime - n, prevoutStake, &nBlockTime))
            {
                ...
                txNew.vout.push_back(CTxOut(0, scriptPubKeyOut, 0));
    ...
    // Calculate coin age reward
    {
        ...
        if((nReward < 0 && COIN_YEAR_REWARD <= 0) || (nReward <= 0 && COIN_YEAR_REWARD >0))
            return false;
        nCredit += nReward;
    }

    if (nCredit >= GetStakeSplitThreshold())
        txNew.vout.push_back(CTxOut(0, txNew.vout[1].scriptPubKey, 0)); //split stake
...
int64_t CWallet::GetUnlockReward(const int64_t nAmount, const int nMonth)
{
    return nAmount * nMonth * UNLOCK_YEAR_REWARD / (12 * COIN);;
}

int64_t CWallet::GetUnlockedTime(const int64_t nStartTime, const int nMonth)
{
    boost::posix_time::ptime t2 = boost::posix_time::from_time_t(nStartTime);

    int year = t2.date().year();
    int month = t2.date().month() + nMonth;
    if(month > 12)
    {
        year += (month / 12);
        month %= 12;
    ...
    int day = t2.date().day();
    int hour = t2.time_of_day().hours();
    int minute = t2.time_of_day().minutes();
    int second = t2.time_of_day().seconds();

    char szTime[50] = {0x00};
    snprintf(szTime, sizeof(szTime), "%04d-%02d-%02d %02d:%02d:%02d", year, month, day, hour, minute, second);
    boost::posix_time::ptime t3 = boost::posix_time::time_from_string(string(szTime));

    boost::posix_time::ptime epoch(boost::gregorian::date(1970,1,1));
    boost::posix_time::time_duration::sec_type x = (t3 - epoch).total_seconds();
    return time_t(x);
}

string CWallet::SendMoneyWithLock(CScript scriptPubKey, int64_t nLockAmount, int nLockMonth, CWalletTx& wtxNew, bool fAskFee)
{
    ...
    if (IsLocked())
    {
        ...
        LogPrintf("SendMoneyWithLock() : %s", strError);
        return strError;
    }
    ...
    int64_t nLockAmountWithReward = nLockAmount + GetUnlockReward(nLockAmount, nLockMonth);
    if(nLockAmountWithReward + nTransactionFee > GetBalance())
        return _("Insufficient funds");

    // Create transaction
    if (!CreateTransactionWithLock(scriptPubKey, nLockAmount, nLockMonth, wtxNew, reservekey, nFeeRequired))
    {
        string strError;
        if (nLockAmountWithReward + nFeeRequired > GetBalance())
            strError = strprintf(_("Error: This transaction requires a transaction fee of at least %s because of its amount, complexity, or use of recently received funds!"), FormatMoney(nFeeRequired));
        else
            strError = _("Error: Transaction creation failed!");
        LogPrintf("SendMoneyWithLock() : %s\n", strError);
        return strError;
    }
    ...
    if(!CommitTransaction(wtxNew, reservekey))
    {
        ...
        mempool.remove(wtxNew);
        CWalletDB* pwalletdb = fFileBacked ? new CWalletDB(strWalletFile,"r") : NULL;
        reservekey.ReturnKey();
        // Erase tx from wallet
        EraseFromWallet(wtxNew.GetHash());
        // Mark old coins as unspent
        set<CWalletTx*> setCoins;
        BOOST_FOREACH(const CTxIn& txin, wtxNew.vin)
        {
            CWalletTx &coin = mapWallet[txin.prevout.hash];
        ...
        if (fFileBacked)
            delete pwalletdb;
        */
        return _("Error: Accept to memory pool failed. Unable to commit transaction");
    }
