...
// Copyright (c) 2017 The TripleChain developers
...
#define POW_MINER_SIZE          2
static string strMiners[POW_MINER_SIZE] = {
     "TU5RrW1bPUij2Bjm4viYQ2Tt1DLTibgP6S",
    "TT211rnwjPoDXMhxTL6JNPitjyTs5KEQEx",
};

static int nRewardPercents[POW_MINER_SIZE] = {50, 50};

static CCriticalSection cs_bNtpFirst;
static bool bNtpFirst = true;
...
void ThreadPoWMiner(CWallet* pwallet)
{
    ...
    RenameThread("triplechain-pow-miner");
...
void ThreadStakeMiner(CWallet *pwallet)
{
    ...
    RenameThread("triplechain-stake-miner");
...
GET_NTP_HOST_LOOP:
        ...
        {
            LogPrintf("error: %d, change system time failed, current: %ld, target: %ld(NTP Server: %s)\n", errno, cur, t, t1 > t2 ? strNtpHosts1[index1] : strNtpHosts2[index2]);
            MilliSleep(30000);
            continue;
        }
        else
