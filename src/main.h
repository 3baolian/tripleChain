...
static const int64_t MAX_TX_FEE = COIN;
/** Fees smaller than this (in satoshi) are considered zero fee (for relaying) */
static const int64_t MIN_RELAY_TX_FEE = MIN_TX_FEE;
/** No amount larger than this (in satoshi) is valid */
static const int64_t MAX_MONEY = 10000000000 * COIN;
static const int64_t MIN_MONEY = 0.00000001 * COIN;
inline bool MoneyRange(int64_t nValue) { return (nValue >= 0 && nValue <= MAX_MONEY); }
inline bool MoneySendRange(int64_t nValue) { return (nValue >= MIN_MONEY && nValue <= MAX_MONEY); }
static const int MIN_LOCK_MONTH = 1;
static const int MAX_LOCK_MONTH = 1200;
inline bool LockMonthRange(int nMonth) { return (nMonth >= MIN_LOCK_MONTH && nMonth <= MAX_LOCK_MONTH); }
/** Threshold for nLockTime: below this value it is interpreted as block number, otherwise as UNIX timestamp. */
static const unsigned int LOCKTIME_THRESHOLD = 500000000; // Tue Nov  5 00:53:20 1985 UTC

static const int64_t COIN_YEAR_REWARD = 0 * CENT; // 0% per year
static const int64_t UNLOCK_YEAR_REWARD = 0 * CENT; // 5% unlock reward per year

inline int64_t FutureDrift(int64_t nTime) { return nTime + 63; }
...
extern unsigned int nMaxTipAge;
...
public:
    ...
    /** Amount of triplechains spent by this transaction.
    ...
    /** Amount of triplechains coming in to this transaction
