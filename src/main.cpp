...
#if defined(NDEBUG)
# error "TripleChain cannot be compiled without assertions."
#endif
...
unsigned int nTargetSpacing = 5;

CBigNum bnProofOfStakeLimit(~uint256(0) >> 20);

unsigned int nStakeMinAge = 2 * 60 * 60; // 2 hours
unsigned int nModifierInterval = 10 * 60; // time to elapse before new modifier is computed
unsigned int nMaxTipAge = 30 * 24 * 60 * 60; // 1 month
...
const string strMessageMagic = "TripleChain Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
...

    {
        ...
        if (nFees < MIN_TX_FEE)
            return error("AcceptToMemoryPool : not enough fees %s, %d < %d", hash.ToString(), nFees, MIN_TX_FEE);
...
bool IsInitialBlockDownload()
{
    ...
    return (GetTime() - nLastUpdate < 63 &&
            pindexBest->GetBlockTime() < GetTime() - nMaxTipAge);
}
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal TripleChain miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Check txout unlock height
            if(IsLockedTxOut(txPrev.vout[prevout.n]))
                return DoS(100, error("ConnectInputs() : txin maybe lock"));
        }
        ...
        if (!IsCoinStake())
        {
            ...
            if (nTxFee < MIN_TX_FEE)
                return fBlock? DoS(100, error("ConnectInputs() : %s not paying required fee=%s, paid=%s", GetHash().ToString(), FormatMoney(MIN_TX_FEE), FormatMoney(nTxFee))) : false;
...
ACCEPT_BLOCK_LOOP:
    ...
    if (IsProofOfStake())
    {
        ...
        if (!CheckProofOfStake(pindexPrev, vtx[1], nBits, hashProof, targetProofOfStake))
        {
            ...
            {
                LogPrintf("prev_block: %s, prev_height: %d, cur_height: %d\n", pindexPrev->phashBlock->GetHex().data(), pindexPrev->nHeight, nHeight);
                LogPrintf("best_block: %s, best_height: %d\n", pindexBest->phashBlock->GetHex().data(), pindexBest->nHeight);
                ...
                {
                    CBlock block;
                    if(!block.ReadFromDisk(prev, true))
                    {
                        prev = prev->pprev;
                        continue;
                    }

                    if(prev->hashProof == uint256())
                    {
                        prev = prev->pprev;
                        continue;
                    }
                    ...
                    CTxDB txdb;
                    if(bRet = block.SetBestChain(txdb, pindexPrev))
                    {
                        LogPrintf("reset best chain to %s:%d\n", block.GetHash().GetHex().data(), prev->nHeight);
                        LogPrintf("current best chain: %s:%d\n", pindexBest->phashBlock->GetHex().data(), pindexBest->nHeight);
                        break;
                    }
                    else
                        prev = prev->pprev;
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if(pfrom)
        LogPrintf("ProcessBlock(): block %s comes from %s\n", hash.GetHex(), pfrom->addrName);
    else
        LogPrintf("ProcessBlock(): block %s\n", hash.GetHex());
    if (mapBlockIndex.count(hash))
    ...
    if(pfrom)
        LogPrintf("ProcessBlock(): block %s comes from %s ACCEPTED\n", hash.GetHex(), pfrom->addrName);
    else
        LogPrintf("ProcessBlock(): block %s ACCEPTED\n", hash.GetHex());
...
bool CBlock::SignBlock(CWallet& wallet, int64_t nFees)
{
    ...
    {
        if (wallet.CreateCoinStake(wallet, nBits, 1, nFees, txCoinStake, key))
        {
...
{
    RenameThread("triplechain-loadblk");
...
int GetHeightFromTxHash(const uint256& hash)
{
    CTransaction tx;
    uint256 hashBlock = 0;
...
bool IsLockedTxOut(const CTxOut& txout)
{
    if(txout.nUnlockHeight > 1500000000)
        return false;
    if(txout.nUnlockHeight <= nBestHeight) // unlocked
        return false;

    int64_t nOffset = txout.nUnlockHeight - nBestHeight;
    if(nOffset <= 28 * 24 * 60 || nOffset > 100 * 365 * 24 * 60) // invalid
        return false;

    return true;
}

bool IsHaveLocked(const CTxOut& txout)
{
    if(txout.nUnlockHeight > 1500000000)
        return false;

    return txout.nUnlockHeight > 0;
}
